...
{
if (id < MIN_STD_ID || id > MAX_STD_ID || s == NULL) {
        return ERR_DB_OP;
    }
...
int add_student(int fd, int id, char *fname, char *lname, int gpa)
{
    ...
    ssize_t bytesRead = read(fd, &existing_student, STUDENT_RECORD_SIZE);
    if (bytesRead == -1) {
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    }

    if (bytesRead > 0 && memcmp(&existing_student, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) != 0) {
...
{
    student_t student;
    // Step 1: Close the original database file
    close(fd);
